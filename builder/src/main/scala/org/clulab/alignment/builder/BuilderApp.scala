...
import org.clulab.alignment.scraper.{DatamartScraper, IsiScraper, ScraperApp, StaticScraperLocations}
...
object BuilderApp extends App {
  ...
  new ScraperApp(new StaticScraperLocations(datamartFileName)).run(Seq(isiScraper))
